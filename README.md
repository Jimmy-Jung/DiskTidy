...
**앱 안에서 설치된 버전을 확인할 수 있다.** DiskTidy는 `LSUIElement` 앱이라 Dock 아이콘도 앱
메뉴도 없다 — 표준 "DiskTidy 정보"로 갈 길이 없어, 지금 무슨 버전을 쓰는지 앱 어디에서도
볼 수 없었다.

- 사이드바 맨 아래, 업데이트 버튼 위에 `버전 1.5.7`을 항상 표시한다. 어느 탭에 있든 보이고
  드래그해서 복사할 수 있다.
- 설정 → 개발자에게 문의에 `현재 버전` 행을 추가했다. 메일 문의는 제목에 버전이 자동으로
  채워지지만 GitHub 이슈에는 직접 적어야 한다.

### 1.5.6에서 달라진 것

**사용 중인 Claude 세션 스크래치와 Codex 빌드 임시파일도 위험을 확인한 뒤 강제 삭제할 수 있다.**
기존에는 사유만 보여 주고 선택 자체를 막아, 실행 중인 작업을 중단하더라도 앱에서는 정리할 수 없었다.

- 사용 중 경고 행도 선택·전체 선택 대상에 포함하고, 하나라도 고르면 세션·빌드 실패 가능성을
  알리는 별도 `강제 삭제` 확인을 거친다.
- 기본 삭제 API는 계속 사용 중 항목을 거부한다. 스캔 때 이미 사용 중으로 표시된 항목에 사용자가
  명시적으로 승인한 경우에만 시간·라이브 상태와 열린 일반 파일 검사를 완화한다.
- 열린 디렉터리 FD나 cwd가 남아 있으면 격리 후 원위치로 복원한다. device·inode·UID·mode,
  하위 트리 소유권·종류·마운트·깊이와 격리 복구 검증은 그대로 유지한다.

### 1.5.5에서 달라진 것

**업데이트 확인이 GitHub 요청 한도에 막혀도 "업데이트 실패"라고만 뜨던 문제를 고쳤다.** 토큰
없는 GitHub API는 IP당 시간당 60회다. 한도를 넘기면 403이 돌아오는데 안내가 `HTTP 403`뿐이라
앱이 고장 난 것으로 읽혔다 — 1.5.4를 설치한 직후 실제로 이렇게 보였다(앱은 실행할 때마다 한
번 확인하므로 개발 중 반복 실행만으로 한도가 찬다).

- 403·429 응답에 `x-ratelimit-remaining: 0`이 함께 오면 한도 초과로 읽고, `x-ratelimit-reset`이
  가리키는 시각을 안내에 적는다 — "GitHub 요청 한도를 다 썼습니다(토큰 없는 요청은 IP당 시간당
  60회). 오후 7:01 이후 다시 확인하세요."
- 남은 횟수가 0이 아닌 403은 그대로 권한 문제로 둔다. GitHub는 접근 권한이 없을 때도 403을
  주므로 상태 코드만으로 단정하지 않는다.
...
- **선택할 수 없는 행은 분모에서 뺀다.** 종료할 수 없는 프로세스처럼 액션이 불가능한 행은 전체 선택 대상이 아니다. 사용 중인 임시파일은 선택 가능하며 강제 삭제 경고를 별도로 띄운다.
...
`/tmp`은 공용 디렉터리다. 최상위에 유닉스 소켓·타 사용자 소유 파일·아직 쓰이는 마커가 섞여 있어서, 일반 항목 하나를 자동 삭제 후보로 올리려면 아래 5개를 **모두** 통과해야 한다. 하나라도 판정할 수 없으면 후보에서 뺀다. 출처가 확인된 에이전트 작업물의 사용 중 경고 행만 아래에 설명한 명시적 예외를 둔다.

<img src="docs/diagrams/temp-safety-gates.svg" width="900" alt="일반 임시파일 자동 후보 판정: 소유자·파일 종류·시간 규칙·lsof 미개방·루트 아님 다섯 규칙을 순서대로 통과하면 후보가 되고, 디렉터리는 하위 트리 전체 통과·마운트 경계·쓰기 권한·깊이 64 조건을 추가로 만족해야 한다. 사용 중 에이전트 작업물의 강제 삭제는 별도 경고와 확인을 거친다">
...
**에이전트 작업물은 후보 단위와 시간 규칙이 다르다.** `/private/tmp/claude-<uid>`는 통째로 잡지 않고 `<프로젝트>/<세션 UUID>` 디렉터리 단위로 내려간다 — 통째로 잡으면 지금 돌고 있는 세션의 작업 파일까지 날린다. 살아 있는 세션·빌드가 참조 중인 DerivedData도 사용 중 경고와 함께 표시된다. 선택해 강제 삭제할 수 있지만, 확인 대화상자가 실행 중인 세션·빌드 실패 가능성을 다시 알린다. 이유는 [`docs/temp-cleanup.md`](docs/temp-cleanup.md) 3-A절.
...
- **사용 중 강제 삭제도 열린 디렉터리 FD나 cwd가 남아 있으면 거부한다.** 격리 뒤 원위치로 복원하며, 열린 일반 파일만 확인 후 삭제할 수 있다.
